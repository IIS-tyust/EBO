...
%             Upper = repmat(upper,2*N,1);
            Upper = repmat(Boundary(1,:),2*N,1);%矩阵重复，以Boundary(1,:)为矩阵元素，重复MaxOffspring行1列，即最后的矩阵为MaxOffspring*D维的全1矩阵
            Lower = repmat(Boundary(2,:),2*N,1);
            Site  = rand(2*N,D) < proM/D;
            mu    = rand(2*N,D);
            temp  = Site & mu<=0.5;
