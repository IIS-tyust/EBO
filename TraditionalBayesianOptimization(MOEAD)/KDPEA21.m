function  [Population,prefitnessP,realIndividual] = KDPEA21(dmodel,Population,prefitnessP,Boundary,reference_vector,Archive_train)
% 返回下一代父本和真实评价的个体
%%
%-------------------参数设置----------------
format compact;
...
% num = 5;
sel_individual = Population(EI_id(1,1:num),:);
sel_prefitness = prefitness(EI_id(1,1:num),:);
sel_ipref = [sel_individual sel_prefitness];
%--------选择第一层实际评价，更新Archive_train---------
nspop1 = [];
rank_info1 = [];
[nspop1,rank_info1] = non_dominant_sort(sel_ipref,m,D); % 对种群按rank标记
realIndividual = nspop1(1:rank_info1(1,1),1:D);
end
